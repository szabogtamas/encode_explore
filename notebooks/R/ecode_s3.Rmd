...
```{r}
genome <- makeTxDbFromGFF(genome_annotation_gtf)
if (remove_gtf) unlink(genome_annotation_gtf)
annotated_GR <- exonsBy(genome, by="gene")
```
...
focused_region_of_genome <- subsetByOverlaps(annotated_GR, region_query)
```

```{r}
plotTranscripts(focused_region_of_genome)
```

## Show results

```{r fig.height=9.6, fig.width=4.8}
plotCoverage(focused_region_of_genome, track_data=local_samples_df, heights=c(4,1))
```
