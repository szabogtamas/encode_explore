...
for (bed in bed_inputs) {
  ...
  rmarkdown::render(
    ...
    params = list(
      ...
      boundary_figure_path = paste(bed_base, "transcriptboundariess.pdf", sep="_")
    )
  )
}
```
...
coverage_tables <- "." %>%
  ...
  bind_rows() %>%
  mutate(
    CONDITION = basename(FILENAME),
    CONDITION = gsub("transcriptboundaries.csv", "", CONDITION)
  )
...
coverage_tables %>%
  ...
  ggplot(aes(x = bins, y = meanCoverage, color = CONDITION, fill = CONDITION)) + 
  geom_ribbon(
    aes(
